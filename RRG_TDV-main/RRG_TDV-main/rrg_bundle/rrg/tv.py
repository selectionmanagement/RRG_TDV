...
from rrg.symbols import short_symbol
...
class TradingViewClient:
    ...
    def get_ohlcv(self, *, symbol: str, resolution: str, bars: int) -> pd.DataFrame:
        ...
        if resolution not in {"D", "W"}:
            raise TradingViewError(f"Unsupported resolution: {resolution!r} (use 'D' or 'W')")
        ...
        ws = self._connect()
        try:
            # TradingView sends an initial session packet; read it before sending commands.
            try:
                ws.recv()
            except Exception:
                pass
            ...
            while True:
                if time.time() - start > self.timeout:
                    raise TradingViewError(f"Timeout while fetching {symbol} ({resolution}, bars={bars})")

                raw = ws.recv()
                for frame in _iter_frames(raw):
                    if frame.startswith("~h~"):
                        # Heartbeat (socket.io): server sends "~m~4~m~~h~1" and expects the same framing back.
                    ...
                    if m == "timescale_update":
                        ...
                        if not isinstance(s1, dict):
                            continue
                        # Newer payload format: list of rows in s1["s"], each has "v" = [time, open, high, low, close, volume?]
                        rows = s1.get("s")
                        ...
                        # Legacy payload format: separate arrays (t,o,h,l,c,v)
                        t_arr = s1.get("t")
