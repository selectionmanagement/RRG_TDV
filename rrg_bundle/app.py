    ...
    for sym, close in closes.items():
        try:
            ...
            if model_id in {"3m_high", "52w_high", "52w_low"}:
                ...
                if len(merged) < max(int(ratio_len), int(mom_len)) + 5:
                    errors[sym] = "Not enough aligned bars"
                    continue
...
def _compute_breadth_new_high_low(
    *,
    ...
    for sym, df in ohlcv.items():
        ...
        new_high = (high > prev_max).astype(float)
        new_low = (low < prev_min).astype(float)
        new_high.name = sym
        new_low.name = sym
...
def main() -> None:
    ...
    st.title(_t("app_title", lang=lang))
    animate_rrg = False
    ...
    with st.sidebar:
        ...
        with tab_visual:
            st.caption(_t("visual_caption", lang=lang))
            theme = "vivid"
    ...
    if table is None or table.empty:
        st.warning(_t("no_symbols_computed", lang=lang))
        return
    ...
    if filtered.empty:
        st.warning(_t("no_symbols_filter", lang=lang))
        return
    ...
    frames_list = _generate_rrg_frames(
        symbol_histories=symbol_histories,
    ...
    if animation_fig is None:
        if frames_list:
            ...
            frame_idx = st.slider(
                ...
                min_value=0,
                max_value=max_idx,
                value=st.session_state.get("rrg_frame_idx", max_idx),
                step=1,
    ...
    if three_month_high_df is None:
        ...
        for sym in symbols:
            ...
            last_close = float(close_series.iloc[-1]) if close_series is not None and not close_series.empty else float("nan")
    ...
    with tab_breadth:
        ...
        lookback_days = int(animation_window) if animation_window else 365
